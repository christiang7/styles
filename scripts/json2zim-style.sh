...
then
   ...
   cp templates/template-zim-style.conf "$jsonFolder"/"$styleFile"
   ...
   for key in "${!tags[@]}"; do
      sed -i "s/{{$key}}/$(yq -r ".${tags[$key]}" "$jsonFolder"/"$jsonFile")/g" "$jsonFolder"/"$styleFile"
   done
   cp "$jsonFolder"/"$styleFile" "$jsonFolder"/style.conf
fi
